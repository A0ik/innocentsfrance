...
export async function POST(request: Request) {
    try {
        const { email, amount, productName, mode, formType, formData, quantity, paymentMethodType } = await request.json();
        ...
        // Méthodes de paiement selon le type demandé
        // 'card' inclut automatiquement Apple Pay et Google Pay dans Stripe Checkout
        if (paymentMethodType === 'sepa_debit') {
            sessionParams.payment_method_types = ['sepa_debit'];
        } else {
            // card = CB + Apple Pay + Google Pay (wallets automatiques dans Stripe Checkout)
