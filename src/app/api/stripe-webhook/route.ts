...
function buildEmailHtml(
    ...
    const isSepa = paymentMethod?.startsWith('Prélèvement SEPA');
    const isApplePay = paymentMethod?.startsWith('Apple Pay');
    const isGooglePay = paymentMethod?.startsWith('Google Pay');
...
export async function POST(request: Request) {
    try {
                ...
                try {
                    ...
                    if (session.payment_intent && typeof session.payment_intent === 'string') {
                        try {
                            ...
                            if (pm?.type === 'card') {
                                const wallet = (pm.card as any)?.wallet?.type;
                                if (wallet === 'apple_pay') {
                                    paymentMethod = `Apple Pay (${pm.card?.brand?.toUpperCase() || 'CB'} •••• ${pm.card?.last4 || '????'})`;
                                } else if (wallet === 'google_pay') {
                                    paymentMethod = `Google Pay (${pm.card?.brand?.toUpperCase() || 'CB'} •••• ${pm.card?.last4 || '????'})`;
                                } else {
                                    paymentMethod = `Carte bancaire (${pm.card?.brand?.toUpperCase() || 'CB'} •••• ${pm.card?.last4 || '????'})`;
                                }
                            } else if (pm?.type === 'sepa_debit') {
                                paymentMethod = `Prélèvement SEPA (•••• ${(pm as any).sepa_debit?.last4 || '????'})`;
                    ...
                    // Pour les abonnements SEPA, le payment_intent peut être null (paiement via SetupIntent)
                    if (!paymentMethod && session.mode === 'subscription') {
                        try {
                            const setupIntentId = (session as any).setup_intent;
        ...
        // ─── Paiement de facture (parrainage) — 1er débit SEPA + renouvellements ──
        if (event.type === 'invoice.payment_succeeded') {
            const invoice = event.data.object as any;
            const billingReason = invoice.billing_reason as string;

            // subscription_create = 1er prélèvement SEPA (peut arriver des jours après le checkout)
            // subscription_cycle  = renouvellement mensuel
            if (billingReason === 'subscription_cycle' || billingReason === 'subscription_create') {
                console.log(`invoice.payment_succeeded (${billingReason}):`, invoice.id);
                ...
                try {
                    ...
                    if (billingReason === 'subscription_create') {
                        // 1er prélèvement SEPA confirmé : email spécifique
                        const amount = invoice.amount_paid;
                        const amountStr = (amount / 100).toFixed(2) + '€';
                        const invoiceUrl = invoice.hosted_invoice_url || '#';
                        const email = formData.email || invoice.customer_email || 'N/A';

                        const html = `
                            <div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;">
                                <h2 style="color:#1a1a2e;border-bottom:2px solid #e5e7eb;padding-bottom:8px;">🏦 Parrainage SEPA — 1er prélèvement confirmé</h2>
                                <p style="color:#16a34a;font-weight:bold;">&#9989; Le prélèvement SEPA initial a été encaissé avec succès</p>
                                <table style="border-collapse:collapse;width:100%;">
                                    <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">Parrain/Marraine</td><td style="padding:8px;border:1px solid #ddd;">${donorName || 'N/A'}</td></tr>
                                    <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">Email</td><td style="padding:8px;border:1px solid #ddd;"><a href="mailto:${email}">${email}</a></td></tr>
                                    ${formData.telephone ? `<tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">Téléphone</td><td style="padding:8px;border:1px solid #ddd;">${formData.telephone}</td></tr>` : ''}
                                    <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">Montant prélevé</td><td style="padding:8px;border:1px solid #ddd;">${amountStr}</td></tr>
                                    <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">Méthode</td><td style="padding:8px;border:1px solid #ddd;">Prélèvement SEPA</td></tr>
                                    <tr><td style="padding:8px;border:1px solid #ddd;font-weight:bold;background:#f9f9f9;">N° Facture</td><td style="padding:8px;border:1px solid #ddd;">${invoice.id || 'N/A'}</td></tr>
                                </table>
                                <p style="margin-top:16px;">
                                    <a href="${invoiceUrl}" style="background:#1a1a2e;color:white;padding:8px 16px;border-radius:6px;text-decoration:none;font-size:14px;">Voir la facture →</a>
                                </p>
                                <p style="color:#9ca3af;font-size:11px;margin-top:16px;">Email généré automatiquement après confirmation du 1er prélèvement SEPA.</p>
                            </div>
                        `;

                        await sendBrevoEmail(
                            `[PARRAINAGE SEPA] 1er prélèvement confirmé — ${donorName || invoice.id}`,
                            html
                        );
                        console.log('Email 1er prélèvement SEPA envoyé:', invoice.id);
                    } else {
                    ...
                    console.error('Erreur email invoice:', emailError);
                }
            }
        }
